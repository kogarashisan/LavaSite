...
P.S.
Beware of skeletons! (use this holy search spell to protect you: Ctrl+F 'skeleton')
*/
...
module.exports = function(grunt) {
	...
	grunt.registerTask('doc_buildDoc', global.bug1135(function() {
		...
		function buildMemberDocs(cd) {
			...
			var skeleton = cd.skeleton,
                class_body = cd.class_body,
				result = {},
				longname,
				is_mixin = (cd.path.indexOf('Lava.mixin.') == 0),
				member_descriptor;

			for (var name in class_body) { // leave only own members
			...
			if (class_body.Shared) {

				var shared_list = typeof(class_body.Shared) == 'string' ? [class_body.Shared] : class_body.Shared;
				shared_list.forEach(function(name) {

					if (!(name in class_body)) throw new Error();
			...
			for (var name in source_object) {
				...
				if (property_descriptors.length) {
					extended_descriptor.member_chain = [{
						descriptors: Firestorm.Sorting[Firestorm.schema.DEFAULT_UNSTABLE_SORT_ALGORITHM](property_descriptors, function(a, b) {
				...
				if (method_descriptors.length) {
					extended_descriptor.method_chain = [{
						descriptors: Firestorm.Sorting[Firestorm.schema.DEFAULT_UNSTABLE_SORT_ALGORITHM](method_descriptors, function(a, b) {
							return a.name < b.name;
						})
					}];
				}
		...
		var Lava = global.Lava,
			...
			LavaBuild = global.LavaBuild,
			fs = require('fs'),
			groups = Firestorm.clone(require(global.LAVA_CORE_DIRECTORY + 'build/files.js')),
			filelist,
			support_names = [],
			missing_descriptions_log = [];

		filelist = groups['classes'].concat(groups['widgets']).concat(groups['data']);
		...
		var getJSDocDescriptor = function(longname, is_function) {
			...
			if (jsdoc_data[longname].kind == 'function' ^ is_function) {
				// @todo exception for single method
				if (longname.indexOf('Firestorm.Sorting.') != 0) {
					throw new Error('doclet is different kind for: ' + longname);
				}
			}
		...
		filelist.forEach(function(file_path){
			...
			var file_content = grunt.file.read(global.LAVA_CORE_DIRECTORY + 'src/' + file_path),
			...
			var class_body = cd.class_body;
			if (('Extends' in class_body) && comment.indexOf('* @extends ' + class_body.Extends) == -1) {
				throw new Error('[Doc] missing or malformed @extends tag: ' + file_path + '. Expecting: * @extends ' + class_body.Extends);
			}
			if ('Implements' in class_body) {
				short_descriptor.implements = typeof(class_body.Implements) == 'string' ? [class_body.Implements] : class_body.Implements;
		...
		// @todo merge these arrays into one
		...
		firestorm_nav_root = Firestorm.Sorting[Firestorm.schema.DEFAULT_UNSTABLE_SORT_ALGORITHM](firestorm_nav_root, function(a, b) {
		...
		function sortDataTree(array) {
			...
			return Firestorm.Sorting[Firestorm.schema.DEFAULT_UNSTABLE_SORT_ALGORITHM](array, function(a, b) {
				if (a.type != b.type) {
		...
		for (var name in extended_class_descriptors) {
			...
			extended_class_descriptor.doc_chain.forEach(function(members_block){
				...
				if (methods.length) {
					methods = Firestorm.Sorting[Firestorm.schema.DEFAULT_UNSTABLE_SORT_ALGORITHM](methods, function(a, b) {
				...
				if (members.length) {
					members = Firestorm.Sorting[Firestorm.schema.DEFAULT_UNSTABLE_SORT_ALGORITHM](members, function(a, b) {
						if (a.is_private != b.is_private) {
							return b.is_private;
						}
						return a.name < b.name;
		...
		missing_descriptions_log = Firestorm.Sorting[Firestorm.schema.DEFAULT_UNSTABLE_SORT_ALGORITHM](missing_descriptions_log, function(a, b) {
